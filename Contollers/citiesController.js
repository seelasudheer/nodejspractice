...
const sendMail = async (req, res, next) => {
  ...
  try {
    ...
    if (data) {
      console.log(data, "duplicate2");
      //  res.send({})
      if(data.length>0)
      return res.send({ status: `Already ${data[0].status}`  });
    }
  } catch (err) {
    console.log(err);
  }
    ...
    try {
      ...
      console.log(email);
        const transporter = nodeMailer.createTransport({
            ...
            secure:false,
            auth: {
              user: "ngracademy99@gmail.com",
              pass: "Moor$000",
            },
          });
          const options = {
            from: "ngracademy99@gmail.com",
            to: req.body.email,
            subject: "Sending mail with node js",
            text: "",
            html:
              '<p> Hi '+ email[0] + ', Click <a href="http://localhost:3000/signup/' +
              randomId +
              "/" +
          ...
          transporter.sendMail(options, async(err, result) => {
            if (err) {
              ...
              res.send({ err, messgae: "mailError" });
              return;
            }
            if (result) {
